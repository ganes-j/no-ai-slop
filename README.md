...
Measured before shipping, by replaying every `Edit`/`Write` in the author's local Claude Code transcripts (a snapshot; the corpus grows with use): **21 new denials across ~1,780 non-denylisted source-file writes (1.2%)**, and **zero change** on the ~2,500 prose-file and ~12,900 other guarded payloads — 156 and 225 denials respectively, before and after. The originating incident (five section definitions written into a `.mjs` as `&mdash;`) moves from allow to deny; entity decoding is load-bearing for it, since without decoding the same payload still passes.
